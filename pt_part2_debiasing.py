...
Original file is located at
    https://colab.research.google.com/github/MITDeepLearning/introtodeeplearning/blob/master/lab2/PT_Part2_Debiasing.ipynb
...
def vae_loss_function(x, x_recon, mu, logsigma, kl_weight=0.0005):
    ...
    latent_loss = 0.5 * torch.sum(torch.exp(2 * logsigma) + mu**2 - 1 - 2 * logsigma, dim=1)
    latent_loss = torch.mean(latent_loss)# TODO
    ...
    reconstruction_loss = torch.mean(torch.abs(x - x_recon).view(x.size(0), -1), dim=1)
    reconstruction_loss = torch.mean(reconstruction_loss)# TODO
...
def sampling(z_mean, z_logsigma):
    ...
    z = z_mean + torch.exp(0.5 * z_logsigma) * eps# TODO
...
def debiasing_loss_function(x, x_pred, y, y_logit, mu, logsigma):
    ...
    vae_loss = vae_loss_function(x, x_pred, mu, logsigma, 0.0005) # TODO
    ...
    # samples
    per_sample_vae_loss = 0.5 * torch.sum(torch.exp(2 * logsigma) + mu**2 - 1 - 2 * logsigma, dim=1)
    per_sample_vae_loss = per_sample_vae_loss + torch.sum(torch.abs(x - x_pred).view(x.size(0), -1), dim=1) / (x[0].numel())
    # (the above expands if you want to log per-sample contributions)
...
def debiasing_train_step(x, y):
    ...
    loss, class_loss = debiasing_loss_function(y_logit, y, x, x_recon, z_mean, z_logsigma) # TODO
